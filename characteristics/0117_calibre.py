        ...
        for status in ['inactive', 'active']:
            ...
            for file in os.listdir(os.path.join(pre_dir, status)):
                if file.startswith(str(patient_number) + "_"):
                    ...
                    calibre = util2.calculate_bvc(img)
                    
                    # Store the entropy value in the DataFrame
                    results_df.at[index, f'{status}_{dirname}'] = calibre
                    break
...
results_df.to_excel('/raid/jupyter-charlielibear.md09-24f36/entropy/excel/calibre/0117_calibre.xlsx')
